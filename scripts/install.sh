...
#搬瓦工：梯子专用机，CN2 GIA线路，高性能，低延迟，99.99%高可用，跨境电商、外贸首选VPS 👉🏻 https://bwh.awesome-vps.com
...
show_client_configuration() {
  ...
  echo ""
  show_notice "VISION_REALITY 通用链接 二维码 通用参数  https://bwh.awesome-vps.com"
  echo ""
  info "通用链接如下"
  echo "" 
  echo "$reality_link"
  ...
  generate_latest_client_configs

  echo ""
  show_notice "最新版 sing-box 客户端 JSON 配置文件"
  info "TUN 模式客户端配置已保存到: /root/sbox/sbconfig_client_tun.json"
  info "本地代理客户端配置已保存到: /root/sbox/sbconfig_client_proxy.json"
  info "Clash Meta / Mihomo 配置已保存到: /root/sbox/clash-meta-client.yaml"
  echo ""
  info "下面打印 TUN 模式客户端 JSON："
  echo "------------------------------------"
  cat /root/sbox/sbconfig_client_tun.json
  echo "------------------------------------"
  echo ""
  info "下面打印本地代理客户端 JSON："
  echo "------------------------------------"
  cat /root/sbox/sbconfig_client_proxy.json
  echo "------------------------------------"
  echo ""
  info "下面打印 Clash Meta / Mihomo YAML："
  echo "------------------------------------"
  cat /root/sbox/clash-meta-client.yaml
  echo "------------------------------------"
  ...
  echo ""
  echo "" 
  show_notice "Hysteria2通用链接 二维码 通用参数  https://bwh.awesome-vps.com"
  echo ""
  info "通用链接如下"
  echo "" 
  echo "$hy2_link"
  ...
  show_notice "Clash Meta / Mihomo 配置已生成"
  info "新版 Clash Meta / Mihomo YAML 已在上面打印，并保存到: /root/sbox/clash-meta-client.yaml"

  show_notice "旧版 sing-box 1.8 客户端配置已省略"
  info "上面已经打印 /root/sbox/sbconfig_client_tun.json、/root/sbox/sbconfig_client_proxy.json 和 /root/sbox/clash-meta-client.yaml 三份新版配置。"

}


# 生成最新版 sing-box 客户端 JSON 和 Clash Meta/Mihomo 配置
# 保持旧脚本风格：直接写入 /root/sbox，并在“显示客户端配置”时打印出来。
generate_latest_client_configs() {
  local server_ip public_key reality_port reality_uuid reality_server_name short_id
  local hy_port hy_server_name hy_password
  ...
  hy_port=$(jq -r '.inbounds[] | select(.tag == "hy2-in") | .listen_port // empty' /root/sbox/sbconfig_server.json)
  hy_server_name=$(grep -o "HY_SERVER_NAME='[^']*'" /root/sbox/config | awk -F"'" '{print $2}')
  hy_password=$(jq -r '.inbounds[] | select(.tag == "hy2-in") | .users[0].password // empty' /root/sbox/sbconfig_server.json)

  cat > /root/sbox/sbconfig_client_tun.json <<EOF
{
  "log": {
    "level": "info",
    "timestamp": true
...
EOF
  ...
  cat > /root/sbox/sbconfig_client_proxy.json <<EOF
{
  "log": {
    "level": "info",
    "timestamp": true
...
EOF
  ...
  cat > /root/sbox/clash-meta-client.yaml <<EOF
mixed-port: 7890
allow-lan: false
mode: rule
log-level: info
...
#搬瓦工：梯子专用机，CN2 GIA线路，高性能，低延迟，99.99%高可用，跨境电商、外贸首选VPS 👉🏻 https://bwh.awesome-vps.com
#丽萨主机：双ISP美国、新加坡、台湾原生住宅IP，tiktok运营，chatGPT/facebook/YouTube/twitter/Netflix等，流媒体全解锁，全新IP段，纯净IP，三网大陆优化线路（CN2 GIA/9929//4827) 👉🏻 https://lisahost.awesome-vps.com
print_with_delay "Reality+Hysteria2 二合一脚本" 0.01
print_with_delay "搬瓦工：梯子专用机，CN2 GIA线路，高性能，低延迟，99.99%高可用，跨境电商、外贸首选VPS 👉🏻 https://bwh.awesome-vps.com" 0.02
